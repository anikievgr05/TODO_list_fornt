...
const ChangeOrdeStatus: React.FC<ProjectAsProps> = ({project}) => {
    const {get_statuses_with_closed, change_order} = status_hook(project)
    ...
    return (
        <OpeningBlock
            title="Поменять порядок статусов"
                        ...
                        <div className="md-2 p-2 w-full">
                            ...
                            <div className="md-2 p-2 w-full text-center text-hot_crimson">
                                ...
                                statuses.map((status, index) => (
                                    <div key={index} className={`md-2 p-2 w-full ${index !== statuses.length - 1 ? 'border-b-2 border-b-deep_onyx' : '' } ${statusChange == 'load' && 'opacity-40'}`}>
...
export default ChangeOrdeStatus