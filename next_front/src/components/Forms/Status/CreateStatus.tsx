...
import {Create} from "@/validations/RoleValidations";
...
const CreateStatus: React.FC<ProjectAsProps> = ({project}) => {
    const { create_status } = status_hook(project)
    const [isLoading, setIsLoading] = useState(false);
    const [status, setStatus] = useState<Status>('empty');

    const submitForm = async (
        values: StatusType,
        { setSubmitting, setErrors, resetForm }: FormikHelpers<StatusType>,
    ): Promise<any> => {
        setIsLoading(true);
        setStatus('load');
        try {
            await create_status(values)
