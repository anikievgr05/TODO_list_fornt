...
import {Create} from "@/validations/PriorityValidations";
...
const CreatePriority: React.FC<ProjectAsProps> = ({project}) => {
    const { create_priority } = priority(project)
    const [isLoading, setIsLoading] = useState(false);
    const [status, setStatus] = useState<Status>('empty');

    const submitForm = async (
        values: Priority,
        { setSubmitting, setErrors, resetForm }: FormikHelpers<Priority>,
    ): Promise<any> => {
        setIsLoading(true);
        setStatus('load');
        try {
            await create_priority(values)
